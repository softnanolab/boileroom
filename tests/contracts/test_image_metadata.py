...
def test_model_specs_report_supported_cuda_from_config() -> None:
    ...
    assert get_supported_cuda(get_model_image_spec("esmc")) == ("11.8", "12.6")
...
def test_iter_image_targets_uses_canonical_cuda_tags() -> None:
    ...
    assert references["esmc"].startswith("docker.io/example/")
    ...
    assert references["esmc"].endswith(":cuda12.6-0.3.0")
