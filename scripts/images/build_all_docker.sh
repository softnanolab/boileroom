...
# Build all Boileroom Docker images locally for development, in dependency order.
#
# Images built:
...
# usage prints the help message showing build order and available options (--platform, --no-cache, -h/--help).
...
BASE_BUILD_CMD=(docker build --platform "${PLATFORM}" -t "${BASE_IMAGE_TAG}" -f "${IMAGES_DIR}/Dockerfile")
...
CHAI_BUILD_CMD=(docker build --platform "${PLATFORM}" --build-arg "BASE_IMAGE=${BASE_ARG_IMAGE}" -t "${CHAI1_IMAGE_TAG}" -f "${CHAI_DIR}/Dockerfile")
if [ "${NO_CACHE}" = true ]; then
